...
const SuggestMealRequestBodySchema = z.object({
  ...
  targetDate: z.string().optional().describe("ISO date string for context, e.g., 2025-12-31"),
  healthGoal: z.string().optional().describe("e.g., weightLoss, muscleGain, balancedDiet"),
  dietaryRestrictions: z.array(z.string()).optional().describe("e.g., vegetarian, glutenFree"),
  allergies: z.array(z.string()).optional().describe("e.g., peanuts, shellfish"),
  dislikedIngredients: z.array(z.string()).optional(),
  calorieTargetForMeal: z.number().int().positive().optional(),
  otherRequirements: z.string().optional().describe("Any other specific user requests"),
...
const SuggestedMealSchema = z.object({
  ...
  recipeId: z.string().optional().describe("ID of the recipe if suggestionType is existing_recipe"),
  name: z.string(),
  ...
  suggestedIngredients: z.array(z.string()).optional().describe("A short list of main suggested ingredients for a new idea"),
  cookingOverview: z.string().optional().describe("A brief cooking overview for a new idea"),
});
...
async function getRelevantExistingRecipes(params: {
  ...
  return recipes.map(r => ({
    ...
    calories: (typeof r.nutritionInfo === 'object' && r.nutritionInfo && 'calories' in r.nutritionInfo) ? (r.nutritionInfo as any).calories : undefined,
  }));
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Invalid request body', details: (error as z.ZodError).flatten().fieldErrors }, { status: 400 });
  }

  const { 
    mealTime, targetDate, healthGoal, dietaryRestrictions, allergies, 
    dislikedIngredients, calorieTargetForMeal, otherRequirements, preferExistingRecipes 
  } = requestData;
  ...
  try {
    ...
    if (preferExistingRecipes) {
      ...
      if (relevantRecipes.length > 0) {
        ...
        existingRecipeContext += 'Please consider these first. If suitable, recommend one or more with their IDs. If not, suggest new ideas.';
      }
    }
    ...
    const aiRequestBody = {
      ...
      messages: [{ role: 'system', content: 'You are a helpful assistant that provides meal suggestions in JSON format.' }, { role: 'user', content: prompt }],
      temperature: 0.7,
    };
    ...
    if (!aiResponse.ok) {
      ...
      return NextResponse.json({ error: `AI suggestion failed: ${aiResponse.statusText}` }, { status: aiResponse.status });
    }
