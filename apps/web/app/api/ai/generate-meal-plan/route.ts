...
const RequestBodySchema = z.object({
  ...
  planDurationDays: z.number().int().positive().min(1).max(30, { message: '计划时长必须在1到30天之间'}),
...
export async function POST(request: Request) {
  try {
    ...
    const { preferences, healthGoal, planDurationDays, dailyCalorieTarget, otherRequirements } = validationResult.data;
...
我的健康目标是：${healthGoal}。`;
    ...
    const requestBodyForAI = {
      messages: [
        {
          content: '你是一个专业的营养师和厨师，擅长根据用户需求制定详细、健康且美味的膳食计划。请严格按照用户提供的格式要求返回结果。',
          role: 'system',
        },
        {
          content: prompt,
      ...
      model: 'doubao-pro-32k', // 或者其他合适的模型
      stream: false,
    ...
    if (!aiResponse.ok) {
      ...
      return NextResponse.json({ error: `AI膳食计划生成失败，API错误: ${aiResponse.statusText}`, details: errorBody }, { status: aiResponse.status });
    }
    ...
    if (aiData.choices && aiData.choices.length > 0 && aiData.choices[0].message && aiData.choices[0].message.content) {
      ...
      return NextResponse.json({ error: 'AI未能生成有效的膳食计划内容。', details: aiData }, { status: 500 });
    }
    ...
    try {
      ...
      const planValidationResult = AiMealPlanSchema.safeParse(generatedMealPlan);
      if (!planValidationResult.success) {
        console.error('AI生成的膳食计划结构校验失败:', planValidationResult.error.flatten());
        return NextResponse.json({ error: 'AI生成的膳食计划格式不正确，无法解析。', details: planValidationResult.error.flatten().fieldErrors }, { status: 500 });
      }
      generatedMealPlan = planValidationResult.data; // 使用经过验证和转换的数据
    } catch (parseError) {
      console.error('解析AI返回的膳食计划JSON失败:', parseError, '\n原始AI响应内容:', mealPlanContent);
      return NextResponse.json({ error: 'AI返回的膳食计划格式错误，无法解析。', details: (parseError as Error).message, rawResponse: mealPlanContent }, { status: 500 });
    }
    // --- AI生成逻辑结束 ---
